...
다니는 일을 예약 에이전트에게 맡겼습니다. 약속하는 건 하나입니다. **나한테
정말 맞는 공고가 올라오면 24시간 안에, 그 공고가 사라지기 전에 알게 된다.**

매일 아침 LinkedIn과 Indeed, 그리고 지금 집중해서 노리는 회사의 채용 페이지를
뒤져 봅니다. 공고마다 제 CV에 적힌 증거와 대조해 100점 만점으로 점수를 매기고,
그중 상위 매칭이 아직 열려 있는지 확인한 뒤, 볼 가치가 있는 것만 이메일로
보내줍니다. 채점 기준표는 감으로 만든 게 아닙니다. **제가 실제로 지원해서
결과가 확인된 과거 실제 지원(Uber와 Google 서류 통과 건 포함)으로 백테스트해서
교정했습니다.** [Claude](https://claude.com) Cowork 모드로 만들고 운영합니다.
이름에 풀려는 문제를 그대로 담았습니다. **사라지기 전에 잡아라(catch before
they vanish)**.
...
흔한 대안과 비교하면 이렇습니다. LinkedIn 저장 검색은 전부 알려주기만 하고
순서를 매겨 주지 않습니다. auto-apply 봇은 내가 고르지도 않았을 곳에 지원서를
뿌립니다. 이 도구는 일부러 그 중간을 노렸습니다. 그리고 **대신 지원하지
않습니다**. 공고를 놓치지 않게 하고 아침마다 검색을 반복하는 수고를 없애는 데서
멈춥니다. 지원할지 말지는 판단이 필요한 일이라 내가 정합니다.
...
있습니다. 만들면서 어떤 결정을 내렸고 무엇이 실패했고 어떻게 되돌렸는지는 따로
길게 정리하고 있습니다. 공개되면 여기에 링크를 추가할게요.
