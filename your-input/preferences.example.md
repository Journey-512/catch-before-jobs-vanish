...
## Recency
Rolling 24-hour window, strict. (Aiming companies widen to 7 days — the
template handles that; nothing to set here.)

## Email threshold
Cutoff 70 (the default; change it freely — see the rubric's "Choosing the
cutoff"). Bands: Top >= 85, Strong 70-84. Postings scoring 60-69 or below are
recorded in the Sheet but never emailed.

## Output language
Sheet and email language: English. (Anything the run writes — including Fit
Reason cells — stays in this one language.)
